...
export class PRStateMachine {
    states = new Map();
    stateFile;
    constructor(stateDir = '/tmp/pr-autopilot-states') {
    ...
    loadStates() {
        try {
            if (fs.existsSync(this.stateFile)) {
                ...
                this.states = new Map(Object.entries(parsed).map(([key, value]) => [key, value]));
            }
        }
        catch (error) {
     ...
     * Transition state
     */
    transition(repo, pr, newState, reason) {
     ...
     */
    markCommentProcessed(repo, pr, commentId) {
        const state = this.getState(repo, pr);
     ...
     */
    isCommentProcessed(repo, pr, commentId) {
        const state = this.getState(repo, pr);
     ...
     */
    getActiveStates() {
        return Array.from(this.states.values()).filter(s => !['BLOCKED', 'FAILED'].includes(s.state));
    }
    /**
