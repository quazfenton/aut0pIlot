...
async function processQueue() {
  ...
  for (const file of files) {
    ...
    try {
      // Use the rawBody if available (exact bytes from GitHub), otherwise reconstruct
      const bodyToSend = entry.rawBody || JSON.stringify(entry.payload);

      // Use the ORIGINAL signature from GitHub
      const originalSignature = entry.headers['x-hub-signature-256'] || entry.headers['X-Hub-Signature-256'];
      
      if (!originalSignature) {
        console.error(`Error: No signature found for ${file}, cannot verify webhook`);
        await fs.rename(filePath, path.join(ERROR_DIR, `${file}.nosig.json`));
        continue;
      }
